...
while ($akun = mysqli_fetch_array($getakun)) 
{
    ...
    if($opsi == 1){
        $stmt = mysqli_query($connect,"UPDATE jurnal_penyesuaian SET no_akun='".$akun['no_akun']."', nama_akun='".$akun['nama_akun']."', debet='$nominal', kredit=0, uraian='$uraian' WHERE no_transaksi='$no_transaksi' ");
    }else{
        $stmt = mysqli_query($connect,"UPDATE jurnal_penyesuaian SET no_akun='".$akun['no_akun']."', nama_akun='".$akun['nama_akun']."', debet=0, kredit='$nominal', uraian='$uraian' WHERE no_transaksi='$no_transaksi' ");
    }
}
